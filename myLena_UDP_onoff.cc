...
main (int argc, char *argv[])
{
  ...
  for (uint16_t i = 0; i < numberOfNodes; i++) {
	OnOffHelper onoff ("ns3::UdpSocketFactory", InetSocketAddress (ueIpIface.GetAddress (i), 50000));
	onoff.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=3]")); 
  ...
  for (uint16_t i = 0; i < numberOfNodes; i++) {
	PacketSinkHelper sink ("ns3::UdpSocketFactory", InetSocketAddress (ueIpIface.GetAddress (i), 50000));
  ...
  p2ph.EnablePcapAll("myLena_UDP_onoff");
  //p2ph.EnableAsciiAll ("myLena");
